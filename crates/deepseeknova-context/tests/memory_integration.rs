...
fn project_memory_loads_deepseeknova_md() {
    ...
    assert!(pm.deepseeknova_md.is_some());
    assert!(pm.deepseeknova_md.unwrap().contains("Project Context"));
}

#[test]
fn project_memory_missing_deepseeknova_md_stays_none() {
    let dir = TempDir::new().unwrap();
    let mut pm = ProjectMemory::new();
    pm.load_deepseeknova_md(dir.path());
    assert!(pm.deepseeknova_md.is_none());
}

//! # DPronix 快速上手示例
//!
//! 运行: `cargo run --example quickstart -p dpronix-cli`
//!
//! 展示 DPronix 核心概念，无需 LLM API key：
...
fn main() -> anyhow::Result<()> {
    ...
    let resolved = dpronix_security::path::sanitize_path(&workspace, safe_path.to_str().unwrap())?;
    println!("   ✅ 安全路径: {}", resolved.display());

    let bad_result = dpronix_security::path::sanitize_path(&workspace, "../../etc/passwd");
    println!("   ⛔ 路径遍历被拒绝: {:?}", bad_result.is_err());
    assert!(bad_result.is_err());

    // ── 2. 安全策略 ─────────────────────────────────────────────
    println!("\n▸ 2. Security — policy checks");
    use dpronix_security::policy::SecurityPolicy;
    ...
    let limits = dpronix_security::limits::ResourceLimits::default();
    println!(

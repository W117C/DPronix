//! Shared test utilities for dpronix-agent integration tests.
...
pub struct MockProvider {
    ...
    tools: HashMap<String, Arc<dyn dpronix_core::Tool>>,
}
...
impl MockProvider {
    ...
    pub fn text(text: impl Into<String>) -> Self {
        Self {
            responses: Mutex::new(vec![vec![
                ...
                Chunk::Usage(dpronix_core::chunk::Usage::default()),
                Chunk::Done,
            ]]),
            tools: HashMap::new(),
        }
    ...
    pub fn with_tools(mut self, tools: Vec<Arc<dyn dpronix_core::Tool>>) -> Self {
        for t in tools {
...
pub struct MockRunner {
    events: Vec<dpronix_core::RunEvent>,
}

impl MockRunner {
    pub fn new(events: Vec<dpronix_core::RunEvent>) -> Self {
        Self { events }
    }
    ...
    pub fn text(text: &str) -> Self {
        Self {
            events: vec![
                dpronix_core::RunEvent::TextDelta(text.to_string()),
                dpronix_core::RunEvent::Done(RunOutput {
                    text: text.to_string(),
                    tool_calls: vec![],
                    usage: None,
                }),
